...
namespace PLSC
{
    ...
    namespace Impl
    {
        ...
        template <class... Cs>
        struct IsCollider<const std::tuple<Cs...>>
        {
            static constexpr bool value = ((Impl::IsCollider<Cs>::value) && ...);
        };

        template <class C, size_t N>
        struct IsCollider<std::array<C, N>>
        {
            static constexpr bool value = Impl::IsCollider<C>::value;
        };

        template <std::invocable F>
        struct IsCollider<F>
        {
            static constexpr bool value = Impl::IsCollider<std::invoke_result<F, void>>::value;
        };
        ...
        struct IsCollider<C>
        {
            static constexpr bool value
                = std::is_base_of_v<Collider::Dummy, C> && !std::is_same_v<Collider::Dummy, C>;
        };

    } // namespace Impl

    template <class... Cs>
    concept IsCollider = Impl::IsCollider<Cs...>::value;

    template <auto... cs>
    concept IsColliderInstance = ((IsCollider<decltype(cs)>) &&...);

    struct AABB : Collider::Dummy
    {
        const vec2 extent, C;
        ...
        inline constexpr std::pair<vec2, vec2> minmax(f32 offset) const
        {
            return {C - extent - offset, C + extent + offset};
        }
    };

    /// Inverse AABB, same data but objects will remain inside the bounding box. Useful for borders, etc.
    struct InvBB : Collider::Dummy
    {
        const vec2 extent, C;
        constexpr explicit InvBB(f32 x0, f32 y0, f32 x1, f32 y1) :
            extent((x1 - x0) * 0.5f, (y1 - y0) * 0.5f), C(x0 + extent.x, y0 + extent.y)
        {
        }
    ...
    struct Circle : Collider::Dummy // TODO: Implement Circle collider
    {
        const vec2 P;
        const f32  r;
    ...
    //        /// need about the same amount of data there isn't much extra overhead by implementing what
    //        could be
    //        /// replaced by a virtual base-class in this way, but in return we get the ability to define and
    //        store
